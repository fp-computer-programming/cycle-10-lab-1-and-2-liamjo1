...
while type(numi) == type(intn): # Active

    nsum = nsum + numi # The current number is added to the sum
...
tlist = [] # Numbers that are divisible by 3 
...
while type(numi) == type(intn): # Always active
    ...
    tlist.append(numi) # Adds the numbers in the list
